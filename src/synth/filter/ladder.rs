...
impl Default for LadderFilter {
    fn default() -> Self {
        Self {
            ...
            ut_1: 0.0,
            out: 0.0,
            IIRLowpass: IirFilter::new_lowpass(44100.0 * 2.0, IIR_DOWNSAMPLING_BANDWIDTH*44100.0/2.0, IIR_DOWNSAMPLE_ORDER),
        }
    }
}
...
impl LadderFilter {
    ...
    pub fn process_sample(&mut self, mut input: f64) {
        ...
        for _ in 0..self.oversampling_factor {
            match self.ladder_integration_method {
                ...
                IntegrationMethod::EulerFullTanh => {
                    self.p0 = self.p0 + self.dt*(tanh_pade32(input - feedback*self.p3) - tanh_pade32(self.p0));
                ...
                IntegrationMethod::PredictorCorrectorFullTanh => {
                    // predictor
                    let p0_prime = self.p0 + self.dt*(tanh_pade32(self.ut_1 - feedback*self.p3) - tanh_pade32(self.p0));
                    let p1_prime = self.p1 + self.dt*(tanh_pade32(self.p0) - tanh_pade32(self.p1));
                    let p2_prime = self.p2 + self.dt*(tanh_pade32(self.p1) - tanh_pade32(self.p2));
                    let p3_prime = self.p3 + self.dt*(tanh_pade32(self.p2) - tanh_pade32(self.p3));

                    // corrector
                    let p3t_1 = self.p3;
                    self.p3 = self.p3 + 0.5*self.dt*((tanh_pade32(self.p2) - tanh_pade32(self.p3)) + (tanh_pade32(p2_prime) - tanh_pade32(p3_prime)));
                    self.p2 = self.p2 + 0.5*self.dt*((tanh_pade32(self.p1) - tanh_pade32(self.p2)) + (tanh_pade32(p1_prime) - tanh_pade32(p2_prime)));
                    self.p1 = self.p1 + 0.5*self.dt*((tanh_pade32(self.p0) - tanh_pade32(self.p1)) + (tanh_pade32(p0_prime) - tanh_pade32(p1_prime)));
                    self.p0 = self.p0 + 0.5*self.dt*((tanh_pade32(self.ut_1 - feedback*p3t_1) - tanh_pade32(self.p0)) + (tanh_pade32(input - feedback*self.p3) - tanh_pade32(p0_prime)));
                },
                // predictor-corrector integration
                // with feedback tanh stage only
                IntegrationMethod::PredictorCorrectorFeedbackTanh => {
                    // predictor
                    let p0_prime = self.p0 + self.dt*(tanh_pade32(self.ut_1 - feedback*self.p3) - self.p0);
                    ...
                    // corrector
                    let p3t_1 = self.p3;
                    self.p3 = self.p3 + 0.5*self.dt*((self.p2 - self.p3) + (p2_prime - p3_prime));
                    self.p2 = self.p2 + 0.5*self.dt*((self.p1 - self.p2) + (p1_prime - p2_prime));
                    self.p1 = self.p1 + 0.5*self.dt*((self.p0 - self.p1) + (p0_prime - p1_prime));
                    self.p0 = self.p0 + 0.5*self.dt*((tanh_pade32(self.ut_1 - feedback*p3t_1) - self.p0) +
                ...
                IntegrationMethod::TrapezoidalFeedbackTanh => {
                    ...
                    let mut x_k = ut;
                    let d_t = c*self.p3 + (b + c*b)*self.p2 + (b.powi(2)+b.powi(2)*c)*self.p1 +
                                (b.powi(3)+b.powi(3)*c)*self.p0 + b.powi(4)*ut;
                    ...
                    for _ in 0..LADDER_MAX_NEWTON_STEPS {
	                    ...
	                    let x_k2 = x_k - (x_k + x_k*tanh_g_xk*c_t - tanh_g_xk - c_t) /
        ...
        if self.oversampling_factor > 1 {
            self.out = self.IIRLowpass.filter(self.out);
        }
    }
    ...
    fn set_integration_rate(&mut self) {
        self.dt = integration_rate(self.sample_rate, self.oversampling_factor, self.cutoff_frequency);
    }
    ...
    pub fn set_sample_rate(&mut self, sample_rate: f64) {
        ...
        self.IIRLowpass.sample_rate = sample_rate * self.oversampling_factor as f64;
        self.IIRLowpass.cutoff_frequency = IIR_DOWNSAMPLING_BANDWIDTH * sample_rate / 2.0;
...
mod Iir {
    ...
    impl IirFilter {
        pub fn new_lowpass(sample_rate: f64, cutoff_frequency: f64, decimator_order: usize) -> Self {
            let mut iir = Self {
                cutoff_frequency,
                sample_rate,
                decimator_order,
        ...
        fn compute_coefficients(&mut self) {
            ...
            for (pa_real, pa_imag) in self.pa_real.iter_mut().zip(self.pa_imag.iter_mut()).take(self.decimator_order/2) {
                *pa_real *= 2.0*PI*Fc; 
                *pa_imag *= 2.0*PI*Fc; 
            }
    
            // bilinear transform to z-plane
            for ((pa_real, pa_imag), (p_real, p_imag)) in self.pa_real.iter_mut().zip(self.pa_imag.iter_mut()).take(self.decimator_order/2)
            ...
            for (((a1, a2), (p_real, p_imag)), k) in self.a1.iter_mut().zip(self.a2.iter_mut()).take(self.decimator_order/2)
            .zip(self.p_real.iter_mut().zip(self.p_imag.iter_mut())).zip(self.k.iter_mut()) {
                *a1 = -2.0* *p_real;
